...
# Public, immutable vocabulary used by the V2 call-profile compiler.  The V1
# envelope deliberately remains BLOCKED/UNKNOWN evidence and never acquires a
# READY state or dispatch authority.
CALLABLE_BLOCKER_REASON_CODES = frozenset(_BLOCKER_REASONS)
CALLABLE_UNKNOWN_REASON_CODES = frozenset(_UNKNOWN_REASONS)
CALLABLE_REASON_CODES = CALLABLE_BLOCKER_REASON_CODES | CALLABLE_UNKNOWN_REASON_CODES
V1_CURRENTNESS_CLOSURE_REASONS = frozenset(
    {
        "CANONICAL_AUTHORITY_NOT_CONFIRMED",
        "CANONICAL_TRANSCRIPT_AUTHORITY_NOT_CONFIRMED",
        "TRUSTED_EVALUATION_TIME_NOT_CONFIRMED",
...
CALLABLE_SURFACE_FIELDS = tuple(
    _ENVELOPE_BODY_FIELDS[
        _ENVELOPE_BODY_FIELDS.index("model_loader_operation") :
        _ENVELOPE_BODY_FIELDS.index("ambiguous_dispatch_replay_allowed") + 1
    ]
)
...
def callable_surface_sha256(envelope: ZeroShotCallableEnvelope) -> str:
    """Hash the exact immutable V1 callable-surface field sequence."""

    if type(envelope) is not ZeroShotCallableEnvelope:
        raise TypeError("envelope must be an exact ZeroShotCallableEnvelope")
    verified = parse_zero_shot_callable_envelope(envelope.to_private_dict())
    document = verified.to_private_dict()
    return _hash({field: document[field] for field in CALLABLE_SURFACE_FIELDS})
...
def callable_plan_currentness_facts(plan: NarrationGenerationPlan) -> dict[str, Any]:
    """Re-evaluate the body-free V1 plan facts used by the V2 compiler."""

    if type(plan) is not NarrationGenerationPlan:
        raise TypeError("plan must be an exact NarrationGenerationPlan")
    preview_sha256, preview_count, preview_reasons = _preview_call_text_facts(plan)
    reasons = tuple(sorted(set(_plan_structure_reasons(plan) + preview_reasons)))
    if reasons or preview_sha256 is None or preview_count is None:
        raise ValueError("plan does not have a current single-preview derivation")
    return {
        "plan_id": plan.plan_id,
        "plan_sha256": _plan_sha256(plan),
        "script_id": plan.script_id,
        "script_text_revision_sha256": plan.script_sha256,
        "model_id": plan.model_id,
        "language_code": plan.language_code,
        "ordered_chunk_manifest_sha256": _ordered_chunk_manifest_sha256(plan),
        "chunk_count": len(plan.chunks),
        "preview_call_text_body_sha256": preview_sha256,
        "preview_text_code_point_count": preview_count,
    }
