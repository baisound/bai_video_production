...
| D3 / TASK-065 | `TASK_LOCAL_DESIGN_ACTIVE / SOURCE_START0`: PL65-B00 and PL65-C01a/C01b are split; preactivation is pinned read/join with local effect zero, post-activation is behind a separate Human Gate. PL-D D01-D09 are source-mapped, but packaged installer CLI `discover` is effectful because it writes installer readback; current source has no authoritative active-install registry/side-by-side selector | every upstream completion receipt; trusted installer/Product current-registration receipt; corrected noncreating TASK-063 reader; Product EXE/payload and lifecycle receipts; TASK-061-B config/history successor binding; accepted design/implementation Gate and later independent Production Activation authority. Root scan and implicit winner remain prohibited | `PL_D_START0 / SOURCE_START0 / EFFECT0` |
...
| TASK-065 design | Option B, PL65-B00, C01a/C01b and PL-D D01-D09 acceptance at exact current head; packaged `discover` is excluded from read-only admission | task-local design active / PL-D START0 / source START0 |
