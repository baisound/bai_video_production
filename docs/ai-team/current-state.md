...
- Owner priority re-route (`2026-08-24`): this developer lane is reordered to `TASK-055 external source-main audit/BVP integration -> TASK-056 final BVP integration -> TASK-029 Human Edit Learning R0 -> TASK-019 Profile Auto-Tuner hosting/integration`. TASK-055 source is fully committed in external `bai-davinci-montage-skills` main at `f8afa412`; BVP R0 exact-schema recovery and fail-closed admission are implemented locally on `codex/task-055-recovery-r0`, with hosting still pending. This sequence supersedes the earlier ordering for this lane only and does not authorize model download, paid/cloud execution, private-media upload, automatic profile/Knowledge Pack promotion, Timeline/Resolve mutation, Release or Deploy.
...
- Owner priority creator-workflow insertion: `TASK-056 / DEV-3 — Chase Keyword Cue / Semantic Audio Cue Bridge`; R0 core and R2 external SKILL integration are implemented; final commit/package pending on latest-main branch `codex/priority-chase-keyword-cue-r1-latest-main`. It reuses existing local FasterWhisper and rational timebase, adds opt-in word timing plus deterministic text-free Montage cue sidecars, and deliberately avoids TASK-036 Product/Shell paths while draft PR #269 owns overlapping local-transcription controls. No model download, cloud/paid provider, canonical Timeline or Resolve authority is implied.
