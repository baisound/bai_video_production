...
def attempt(job: str, *, strategy=RegenerationStrategy.TEXT_PROMPT, failures=("DEPTH_ORDER",), result=GenerationResult.FAIL, parent=None):
    return GenerationAttempt(job, "slot-1", "prompt-1", 1, SHA, "provider-1", "model-1", strategy, result, failures, parent_attempt_id=parent)
...
def test_attempt_requires_exact_registered_prompt_hash():
    ...
    bad = GenerationAttempt("job-1", "slot-1", "prompt-1", 1, "sha256:" + "d"*64, "provider-1", "model-1", RegenerationStrategy.TEXT_PROMPT, GenerationResult.FAIL)
    with pytest.raises(ProductError) as exc:
