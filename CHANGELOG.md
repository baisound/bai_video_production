...
## [Unreleased]
- knowledge/commentary向けmain-only checkout probeとfixture pipeline、montage向けinstalled exact3 test seamとTASK-055／056→receipt／advisory Profile→Resolve handoff fixtureを追加しました。probeはbranch／HEAD／dirty state／required entrypointをfail closedに検証し、両fixtureはreview-only／non-authoritative境界を維持します。実素材runtime、canonical Timeline、Resolve write、Release／Deploy／Production authorityは生成しません。
...
- TASK-052として、DbD設計・実装ギャップ監査、分類体系とHuman-first詳細・安全削除、マップ画像decode/回転、Survivor主体Observation/Gold/Teacher契約、transactionalな一括画像登録、発電機・主体別追跡・Survivor状態・フック回数・Killer固有effect・正負Status Effectのprofile-bound時系列状態機械、confirmed Temporal Decisionからadmitted-Evidence CGELへの主体別統合、Killer識別後だけ専用Detectorを選ぶCapability RegistryとGhost Face/Onryo/Doctor starter fixture、既存HUD Profile/recorded-videoからの主体・digest Evidence付きrouting、Capability-boundなKiller固有positive/hard-negative Teacher manifest/indexとstarter reference Detector、登録済みCapability/role/namespace/stateだけを許可するSafe Visual Learning/Training Studio一括Teacher UI、HUD Profile 2.3の正負Status Effect領域と0/1/複数iconのbounded body-free segmentation、登録済み定義から極性・出所を解決するStatus Effect identity/visibility認識、正負Status Effect Teacher manifest/Safe Visual Learning/index admission、Status Effect専用Gold指標・Human correction provenance・R3C temporal bridgeを追加しました。別KillerのHUD namespace、識別UNKNOWN、ROI/Evidence欠落、異常stage、正負region交差、Perk hard-negative、曖昧一致、crop digest不一致、未登録Teacherは専用認識へfall throughせずUNKNOWN、contradictionまたは明示的なunavailable/overflowになり、共通Survivor sliceは重複抽出せず再利用します。Killer固有TeacherとStatus Effect Teacherはpreview receiptからHuman確認・一括commit・index再構築まで契約を保持し、汎用編集による契約破壊を拒否します。Status Effect時系列はregistry・主体・領域Evidenceが完全なIDENTIFIEDだけを受け入れ、UNKNOWNや不完全regionから消失を推測しません。実機Windows EXEでネストしたKnowledge詳細が起動を停止する`unhashable type: 'dict'`も修正し、Ownerワークスペースで起動・検索・主要画面操作を確認しています。workspace別・revision付きStatus Effect定義Registryと正負Teacher UIも追加し、単一アイコン確認のないwhole-region登録を拒否します。全体のreal-media packaged acceptance、Provider、Release、Deployは別Gateです。
- TASK-052 R6-R9として、Tier 3物体/シーン認識、明示Authority/費用上限/Evidence付きの選択的Vision計画、5-10試合Human Gold・correction・rejection・per-domain KPI契約を追加しました。Human Goldは実メディア・権利・Human labeler provenanceと最低precision/recall/UNKNOWN/calibration/replay品質を要求し、secret参照やAuthorityによる失敗KPIの上書きを拒否します。Windows実機で正確なEXEを再ビルドし、起動、日本語検索、HUD/画像学習/Review/Backup画面、cached PNGの0/90/180/270度回転とreopen/restart永続化、isolated backup/restore契約を確認しました。実動画multi-Killer/UI-scale、完全Human Gold、production accuracyはHuman/data gateのためNOT_CONFIRMEDです。
