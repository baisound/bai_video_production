...
Status: `SOURCE_ALLOCATED_R1 / DEV-4 / A1_EFFECT_0 / TASK068_DEPENDENCY_NC`

Design identity: `TASK072-A-OP-TICKET-CORE-V1-R1`
...
- allocated unit: `TASK-072-A1 AUTHORITY_ZERO_AUDIT_CONTRACT_CORRECTION_R1`;
- current sole Builder: `Development 2 / Owner-proxy Recovery transfer`;
- predecessor holder: `GitHub metadata infrastructure task
  01a04b84-6f59-7403-82d2-64bcea5f5cd1` (`BROKEN_THREAD`, historical only);
- logical active lock: `TASK-072-A1-AUTHORITY-ZERO-CORRECTION-R1`;
- current authority ceiling: this exact A1 source allocation only;
- source, schema, test and task-local Evidence effects: authority-zero only;
  native, package, Release, Deploy and Production effects: `0`.
...
This R1 A1 Atomic Unit may change only:

- `docs/ai-team/tasks/TASK-072/a1-schema-public-audit-implementation-evidence.md`;
...
- `tests/fixtures/task072/operation-port-v1/action-profiles.json`;
...
- explicit Owner-proxy source allocation is active only for the exact files in
  section 2, exact base `b7b2f33f9acca95b5bf0d727361f0e794a2d5f82`, the
  sole Builder and logical lock recorded above;
- PR #515 exact head `6a0d11ac985c7cb4b420cf1a8980b083379dfec6` is a
  preserved logic candidate only.  It supplies no source, merge, lock or
  completion authority and must not receive another push;
- TASK-068 source is present on main, but its completion documents remain
  stale and do not provide a current accepted downstream binding.

Before the A1 R1 commit, fresh origin/main/worktree/dirty/open-PR/lock checks,
focused negative verification, exact scope review, independent Critic/Tester
and Judge remain required.
