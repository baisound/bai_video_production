...
| D2.5 / SKILL-D2S-001 Production transport | **SUPERSEDED source-reference wording:** the foreign `installer-relative-learning-bridge@c86ec8c` dirty eight-file worktree remains preserve-only and is not a receipt. Dedicated clean-main D2S Draft PR #8 at `6a391336` is clean and retains useful partial work (disabled `bridge_root:null` sentinel, local no-replace delivery, v2 data ABI and privacy/JSON test additions), but independent review is **HOLD** (`C/H/M/L=0/2/4/0`): H1 permits direct enabled-v1 CLI staging without a live consume/burn path and H2 does not pin full ancestors to opened handles. Neither canonical main/release/install exact read-back nor a BVP completion receipt exists | correct H1/H2 and M1--M4 on the frozen D2S branch, rebind independent review and Windows test evidence, then require canonical main/release/install exact-byte read-back and a Product-owned completion receipt. PL-A rebind only after the released installed result | `D2S_PR8_HOLD / H1_ONE_SHOT_AUTHORITY.N.C. / H2_HANDLE_PINNING.N.C. / RELEASE_INSTALL_READBACK.N.C. / PARTIAL_HUNKS_ARE_NOT_RECEIPT / EFFECT0` |
