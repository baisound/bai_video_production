...
.production-drawer{position:fixed;z-index:30;top:46px;right:0;bottom:0;width:min(540px,100vw);padding:14px;background:#0d1117f7;border-left:1px solid var(--line);box-shadow:-12px 0 30px #0009;overflow:auto}.production-drawer[hidden]{display:none}.production-heading{display:flex;align-items:center;gap:10px;margin-bottom:12px}.production-heading h2{margin:0;font-size:17px}.production-summary{color:var(--muted);margin:0 0 12px}.production-slot{border:1px solid var(--line);border-radius:8px;background:var(--panel);margin-bottom:10px;overflow:hidden}.production-slot-head{display:flex;justify-content:space-between;gap:8px;padding:10px 12px;background:var(--panel2)}.production-candidate{padding:10px 12px;border-top:1px solid var(--line)}.production-meta{color:var(--muted);font:11px/1.45 monospace;white-space:pre-wrap;overflow-wrap:anywhere}.production-lock{margin-top:8px}.production-empty{padding:18px;border:1px dashed #394553;border-radius:8px;color:var(--muted)}@media(max-width:900px){.production-drawer{top:92px}}
</style></head>
<body><a class="skip-link" href="#transcriptRows">編集内容へ移動</a><div class="app">
<header class="top"><div class="brand">BAI Video Production</div><div class="project" id="projectName">プロジェクト未選択</div><button class="workspace active" data-w="EDIT">編集</button><button class="workspace" data-w="SUBTITLE">字幕</button><button class="workspace" data-w="REVIEW">レビュー</button><button class="workspace" data-w="PRODUCTION_CONTROL">制作管理</button><button class="workspace" data-w="EXPORT">書き出し</button><div class="spacer"></div><span id="dialogStatus" class="dialog-status" role="status" aria-live="polite">選択操作なし</span><span id="job">待機中</span><button class="action" id="workflowActionButton" aria-label="次の編集工程を実行" disabled>Continue</button><button class="action" id="chooseProjectButton" aria-label="プロジェクトフォルダーを選択">プロジェクト</button><button class="action" id="chooseMediaButton" aria-label="メディアファイルを選択">メディア</button><button class="action" id="chooseHandoffButton" aria-label="EDITOR WORK保存先を選択">保存先</button></header>
