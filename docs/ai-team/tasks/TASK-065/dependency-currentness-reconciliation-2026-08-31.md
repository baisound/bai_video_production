...
| D2.5 / TASK-036 | `START0`: waits for TASK-061-A, TASK-063, SKILL-D2S-001 and TASK-067 receipts. Source confirms the actual chain `task036_shell.spec -> task036_windows_entry.py -> packaged_main()`, one-dir `COLLECT`, `console=False` and final `BAI Video Production.exe`; existing tests are static chain/spec/output-name checks only | packaged exact single-record route at `packaged_main`, frozen Montage entry/module and payload-tree inclusion contract, internally resolved installed EXE, private dispatch exact1 with probe/guard/shell/presenter0 and installer/discover0, real-installed execution plus durable body-free receipt/correlation/Profile currentness. stdout/exit0/EXE presence are ineligible | `START0 / EFFECT0` |
