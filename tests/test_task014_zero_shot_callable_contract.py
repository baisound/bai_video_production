...
def test_v1_currentness_closure_reasons_remain_unknown_and_explicit() -> None:
    envelope = compile_ready(assembled())
    assert envelope.decision is CallableEnvelopeDecision.UNKNOWN
    assert frozenset(envelope.reason_codes) == V1_CURRENTNESS_CLOSURE_REASONS
    assert V1_CURRENTNESS_CLOSURE_REASONS <= CALLABLE_REASON_CODES


def test_callable_surface_hash_is_exact_body_free_projection() -> None:
    envelope = compile_ready(assembled())
    document = envelope.to_private_dict()
    expected = sha256_bytes(
        canonical_json_bytes({field: document[field] for field in CALLABLE_SURFACE_FIELDS})
    )
    assert callable_surface_sha256(envelope) == expected
    assert set(CALLABLE_SURFACE_FIELDS).isdisjoint(
        {"script_text", "reference_audio", "reference_transcript", "output_path"}
    )


def test_callable_plan_currentness_facts_recompute_preview_and_chunks() -> None:
    parts = assembled()
    facts = callable_plan_currentness_facts(parts["plan"])
    envelope = compile_ready(parts)
    assert facts["plan_sha256"] == envelope.plan_sha256
    assert facts["preview_call_text_body_sha256"] == envelope.preview_call_text_body_sha256
    assert facts["preview_text_code_point_count"] == envelope.preview_text_code_point_count
    assert facts["chunk_count"] == 1

    tampered = replace(
        parts["plan"],
        chunks=(replace(parts["plan"].chunks[0], text="別本文"),),
    )
    with pytest.raises(ValueError, match="single-preview derivation"):
        callable_plan_currentness_facts(tampered)
