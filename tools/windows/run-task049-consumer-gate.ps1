...
try {
  ...
  $mainBuildRoot = Join-Path $runtimeRun 'main-build'
  ...
  $receipt = [ordered]@{
    ...
    paths = [ordered]@{ worktree = $repo; runtime_root = $runtimeRun; evidence_root = $evidenceRun }
    packages = [ordered]@{
    ...
    intentional_residual_artifacts = @($runtimeRun, $evidenceRun)
  }
