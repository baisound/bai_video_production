...
After the existing TASK-063 post-install dispatch has successfully provisioned and read back its descriptor, the main installer invokes the TASK-066 helper exactly once. The helper calls the accepted GF-A InstallLayout resolver/validator, consumes the existing descriptor's exact `install_instance_id`, provisions `desktop-install-layout.json` plus the protected `settings`, `logs` and `runtime-cache` leaves, writes the fixed `<data_root>\settings\installation\task066-installer-readback.json` receipt, and performs its own exact sidecar/leaf/receipt read-back before returning success. Receipt creation is containment-checked, ancestor-reparse-safe, regular-file-only, hardlink-rejecting, atomic and no-clobber unless the exact prior Product receipt is validated for repair. Inno Setup consumes only the helper's exit status; it never parses or independently validates JSON, DACL, root or sidecar meaning.

`settings\installation` is a GF-E installer-owned durable subtree, separate from the main Settings service's `desktop-compute-profile.json` ownership; the profile writer cannot create, replace or delete installer lifecycle records. It is excluded from every runtime-cache/log cleanup. Normal uninstall deliberately preserves the InstallLayout sidecar, receipt and the physical `settings`, `logs` and `runtime-cache` data leaves. The binary-root sidecar and every data-root lifecycle/user-data record are outside Inno `[Files]` payload tracking and every uninstall/delete directive. GF-E adds no `[UninstallDelete]` entry and requires no helper mutation during normal uninstall; payload tracking may remove the helper EXE only. Physical deletion requires a separate exact owner-data cleanup Gate.

A genuinely first install requires both TASK-066 InstallLayout sidecar and durable receipt to be absent after the current TASK-063 descriptor is validated. `REINSTALL_READBACK` is a separate read-only branch for normal-uninstall preservation: it admits only the exact same-instance current TASK-063 descriptor, TASK-066 sidecar and durable receipt with matching schema/version, binary-root identity, layout/receipt digests and currentness, then revalidates rather than overwrites them. Upgrade/repair applies its own prior-receipt transaction. Inno registration state alone never selects first-install, reinstall, upgrade or repair.

First-install, upgrade and repair use the durable `<data_root>\settings\installation\task066-installer-transaction.json` journal; the sidecar and receipt are never described as jointly atomic. The self-hashed journal binds schema/version, operation ID and phase, `install_instance_id`, canonical binary/data roots, exact predecessor identities where applicable, intended sidecar/receipt hashes, helper/build identity and currentness. The journal is atomically published, flushed and exactly read back before any sidecar or receipt mutation begins. Every individual journal, sidecar and receipt write uses same-directory temp, flush, atomic rename and exact read-back.

The sole commit point is atomic publication and exact read-back of the new durable receipt after the intended sidecar was independently read back. A pending exact journal may be resumed only as that journaled operation after revalidating every bound identity: absent or matching intended sidecar rolls forward; rollback means only removing exact current-operation temp/pending artifacts and never restoring, deleting or changing a committed predecessor; a valid committed receipt plus matching sidecar finalizes/cleans a leftover journal without rewriting Product state. If exact roll-forward is impossible, recovery fails closed rather than reconstructing or restoring from ambient files. No journal, foreign/stale journal, operation mismatch or unbound mixed state has recovery authority and remains fail-closed. Previous committed receipts are never deleted as rollback.

Any one-sided predecessor, claimed-first predecessor, missing/stale/cross-instance/substituted/tampered/reparse/nonregular/hardlinked receipt or sidecar, or binary-root/layout mismatch fails only that lifecycle operation closed and never triggers reconstruction from ambient files.

The helper is not a general CLI and accepts only bounded first-install provision/read-back, `REINSTALL_READBACK`, repair/upgrade transaction and journal recovery operations. Normal uninstall performs no helper mutation. Each outer installer operation may launch the helper once, waits at most `30 seconds`, and never retries automatically. Timeout may terminate only the exact helper process started and owned by that operation; unknown ownership is not killed. It has no network, model, provider, UI, GPU execution, TASK-063 bridge mutation, physical user-data deletion or Release authority. A timeout or helper/receipt mismatch returns nonzero and fails the exact lifecycle operation closed without claiming success; recovery preserves pre-existing user data and exact TASK-063 state.
...
Lifecycle tests additionally prove runtime-cache cleanup cannot remove the durable receipt, the profile writer cannot mutate it, normal uninstall adds no delete rule and preserves sidecar/receipt/data without helper mutation, exact uninstall→`REINSTALL_READBACK` succeeds without overwrite, and upgrade/repair validates every predecessor coordinate. Fault injection covers power loss, crash, timeout and owned-helper termination before/after every journal, sidecar, receipt and commit phase on same-volume and cross-volume roots; only exact journaled roll-forward/owned-precommit rollback succeeds. Claimed-first predecessor, one-sided/mixed state without its exact journal, missing/stale/cross-instance/substituted/tampered/reparse/nonregular/hardlink or binary-root/layout mismatch rejects without ambient reconstruction.

The main executable build is GF-E packaging work, not GF-F observation work. After the TASK-063 terminal handoff and exact locks, root `build-windows-exe.bat` follows the existing TASK-059 helper precedent with a separate TASK-066 identity: build the dedicated helper from its exact spec into a unit-scoped clean helper dist/work root; verify fixed filename, absolute regular non-reparse/non-hardlink identity, size and SHA-256; set only `BVP_TASK066_GPU_INSTALLER_HELPER_EXE`; and invoke `task036_shell.spec`. The spec independently validates that identity, generates the expected helper hash identity, and COLLECTs exactly one fixed-name helper into the newly built main onedir. The batch then verifies the bundled helper count/name/type/hash against the source helper and performs only a secret-free bounded helper smoke. Ambient or prior `dist` content is never an input.

`tools/windows/build-task063-main-installer.ps1` consumes only that exact freshly built main onedir as `PayloadRoot`. Before compiler invocation it verifies exactly one fixed-name helper, regular/non-reparse/non-hardlink type, size and SHA-256 against the generated build identity, then computes the payload-tree hash after helper composition. Its build receipt binds source HEAD, TASK-066 helper spec hash, main spec hash, helper filename/hash/size, canonical `PayloadRoot` identity, payload-tree hash and installer artifact hash.

`task063_main_installer.iss` contains an explicit compile-time fixed-source check for the helper path; the existing wildcard copy is not proof. The joint pre-compile contract rejects missing/wrong filename, directory/reparse/nonregular/hardlink, duplicate, substituted or hash-drifted helper before Inno compilation. Inno does not download, build, substitute or parse TASK-063/TASK-066 JSON semantics. Focused contract tests cover explicit build/COLLECT/package ordering, isolated roots, exact name/hash read-back, stale ambient output ignored, all listed negative cases, receipt binding and absence of GF-F build ownership. GF-F receives only the frozen build receipt for later separately authorized native observation.

Separate compile/runtime tests must prove that the binary-root sidecar and all data-root lifecycle/user-data records are outside `[Files]` payload tracking and every uninstall/delete directive, while the helper EXE alone remains installer-managed payload.
...
- Post-acceptance build-path delta: the current main-installer build script neither built nor composed the helper, while the ISS wildcard permitted compilation without it. The source-backed GF-E build/COLLECT/pre-Inno/ISS chain above closes that gap. Independent delta-2 Critic `C/H/M/L = 0/0/0/0`; independent delta-2 Judge `ACCEPT`. Implementation remains gated by TASK-063 terminal handoff and fresh currentness/overlap/exact locks.
- Post-acceptance lifecycle delta: the repair predecessor receipt was initially placed under cleanup-eligible `runtime-cache`. The durable installer-owned `settings\installation` binding and explicit `REINSTALL_READBACK` lifecycle above close that gap. Independent delta-3 Critic `C/H/M/L = 0/0/0/0`; independent delta-3 Judge `ACCEPT`. The Critic's final file-reader retry was unavailable, so its final verdict binds the reviewed patch packet plus its preceding direct file audit; Judge direct read-back and root verification remain separate Evidence.
- Post-acceptance transaction/recovery delta: sidecar and receipt may live on different volumes and normal uninstall preserves both. The journal-first, receipt-only commit, exact roll-forward/owned-temp cleanup, payload/delete exclusion and four-operation helper contract above replace every joint-atomic or uninstall-helper implication. Independent delta-4 Critic `C/H/M/L = 0/0/0/0`; independent delta-4 Judge `ACCEPT`. TASK-063 repair source is canonical, but its post-repair native install/read-back terminal handoff is not yet confirmed, so GF-E installer mutation remains blocked pending that handoff plus fresh currentness, overlap and exact locks.
