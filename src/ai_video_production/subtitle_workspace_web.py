...
<title>BAI Subtitle Workspace</title><style nonce="__NONCE__">body{font:15px/1.5 system-ui;margin:auto;max-width:1200px;padding:20px;background:#f5f7fa;color:#17202a}button,input,textarea{font:inherit}button{padding:8px 12px;margin:3px;border:0;border-radius:7px;background:#1769e0;color:#fff}button.danger{background:#b42318}.bar,.cue{background:#fff;border:1px solid #d5dde5;border-radius:10px;padding:12px;margin:10px 0}.row{display:grid;grid-template-columns:130px 130px 1fr auto;gap:8px;align-items:start}.row input,.row textarea{width:100%;padding:7px;box-sizing:border-box}.muted{color:#657382}.ok{color:#18794e}.error{color:#b42318}@media(max-width:700px){.row{grid-template-columns:1fr}.row button{width:100%}}</style></head>
...
<section class="bar"><input id="importPath" size="55" placeholder="D:\path\subtitles.srt"><button id="import">SRTを読込</button><input id="exportPath" size="55" placeholder="D:\path\edited.srt"><button id="export">SRTを書出</button><label><input id="ai" type="checkbox"> AI誤字・脱字チェックを許可（既定OFF）</label><span id="msg"></span></section>
...
document.querySelector('#import').onclick=async()=>{try{await call({operation:'import_srt',path:document.querySelector('#importPath').value});show('SRTを取り込みました')}catch(e){show(e,true)}};document.querySelector('#export').onclick=async()=>{try{await call({operation:'export_srt',path:document.querySelector('#exportPath').value});show('SRTを書き出しました')}catch(e){show(e,true)}};
...
def launch_server(service: SubtitleWorkspaceWebService, port: int = 8770):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _valid(self): return urlsplit(self.path).path in {"/","/api/workspace"} and self.headers.get("Host") in {f"127.0.0.1:{port}",f"localhost:{port}"}
        def do_GET(self):
        ...
        def do_POST(self):
            try:
                ...
                result=service.apply(json.loads(self.rfile.read(size)))
                self._send(200,json.dumps(result,ensure_ascii=False).encode())
            except (ValueError,KeyError,TypeError,json.JSONDecodeError) as exc: self._send(400,json.dumps({"message":str(exc)},ensure_ascii=False).encode())
        def log_message(self,*args): pass
    server=ThreadingHTTPServer(("127.0.0.1",port),Handler); thread=threading.Thread(target=server.serve_forever,daemon=True);thread.start();return server,thread,f"http://127.0.0.1:{port}/"
