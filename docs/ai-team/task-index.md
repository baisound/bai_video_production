...
| TASK-010 | Resolve Assembly MVP | IMPLEMENTED_NATIVE_RESOLVE_VALIDATION_PENDING | OWNER_DIRECTED_IMPLEMENTATION | DEV-4 | Approved Edit Plan -> TASK-022 mapping -> Automation-owned BAI_AUTO Timeline; explicit write authorization + marker idempotency; real source FPS mandatory |
| TASK-011 | Render QA / Loudness | IMPLEMENTED_NATIVE_RENDER_VALIDATION_PENDING | OWNER_DIRECTED_IMPLEMENTATION | DEV-3/4 | Render artifact checksum/probe/duration + configurable LUFS/true-peak QA; path-free report; native Resolve render flow remains acceptance gate |
| TASK-012 | Manual Handoff / Cubase | IMPLEMENTED_NATIVE_HANDOFF_VALIDATION_PENDING | OWNER_DIRECTED_IMPLEMENTATION | DEV-3 | QA-gated deterministic EDITOR_WORK package + bounded 48 kHz PCM Cubase return; no automatic Cubase project conversion claim |
