...
<header class="top"><div class="brand">BAI Video Production</div><div class="project" id="projectName">プロジェクト未選択</div><button class="workspace active" data-w="EDIT">編集</button><button class="workspace" data-w="PLANNING">企画</button><button class="workspace" data-w="GENERATION_SAFETY">生成安全</button><button class="workspace" data-w="CONTINUITY">連続性</button><button class="workspace" data-w="SUBTITLE">字幕</button><button class="workspace" data-w="REVIEW">レビュー</button><button class="workspace" data-w="PRODUCTION_CONTROL">制作管理</button><button class="workspace" data-w="EXPORT">書き出し</button><div class="spacer"></div><span id="dialogStatus" class="dialog-status" role="status" aria-live="polite">選択操作なし</span><span id="job">待機中</span><button class="action" id="workflowActionButton" aria-label="次の編集工程を実行" disabled>Continue</button><button class="action" id="chooseProjectButton" aria-label="プロジェクトフォルダーを選択">プロジェクト</button><button class="action" id="chooseMediaButton" aria-label="メディアファイルを選択">メディア</button><button class="action" id="chooseHandoffButton" aria-label="EDITOR WORK保存先を選択">保存先</button></header>
...
async function refresh(){const vm=await call('view_model');const x=vm?.shell||await call('snapshot');if(!x)return;const p=x.project;document.querySelector('#projectName').textContent=p?p.display_name:'プロジェクト未選択';document.querySelector('#job').textContent=x.active_jobs?.length?`${x.active_jobs.length} job`:'待機中';document.querySelectorAll('.workspace').forEach(b=>b.classList.toggle('active',b.dataset.w===x.current_workspace));if(vm){renderRows(vm);renderTimeline(vm)}const review=await call('review_snapshot');if(review)renderReview(review);const drawer=document.querySelector('#productionWorkspace'),planningDrawer=document.querySelector('#planningWorkspace'),safetyDrawer=document.querySelector('#generationSafetyWorkspace'),continuityDrawer=document.querySelector('#continuityWorkspace');if(x.current_workspace==='PRODUCTION_CONTROL'){const production=await call('production_snapshot');const audit=await call('audit_snapshot');renderProduction(production,audit);planningDrawer.hidden=true;safetyDrawer.hidden=true;continuityDrawer.hidden=true}else if(x.current_workspace==='PLANNING'){const planning=await call('planning_snapshot');renderPlanning(planning);drawer.hidden=true;safetyDrawer.hidden=true;continuityDrawer.hidden=true}else if(x.current_workspace==='GENERATION_SAFETY'){const safety=await call('generation_safety_snapshot');renderGenerationSafety(safety);drawer.hidden=true;planningDrawer.hidden=true;continuityDrawer.hidden=true}else if(x.current_workspace==='CONTINUITY'){const continuity=await call('continuity_snapshot');renderContinuity(continuity);drawer.hidden=true;planningDrawer.hidden=true;safetyDrawer.hidden=true}else{drawer.hidden=true;planningDrawer.hidden=true;safetyDrawer.hidden=true;continuityDrawer.hidden=true}const runtime=await call('workflow_status');const action=document.querySelector('#workflowActionButton');action.disabled=!runtime?.available||!['media.choose_and_ingest','transcription.start','subtitle.save','cut_candidates.generate','resolve.assembly.prepare','resolve.assembly.apply','render.start','render.qa.inspect','handoff.create'].includes(runtime.next_recommended_action);action.textContent=runtime?.next_recommended_action||'Continue'}
