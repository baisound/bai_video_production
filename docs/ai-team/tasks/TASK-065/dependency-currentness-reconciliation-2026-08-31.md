...
| --- | --- | --- | --- |
| D0 / TASK-063 | initial installer-relative `20f5360`, read-back boundary `0b95e40` and publication race/path-safety `8fd17ed` are canonical; task status remains `OWNER_AUTHORIZED / IMPLEMENTATION_ACTIVE` | post-`8fd17ed` re-provisioned installed read-back, DACL/reparse/hardlink negatives, descriptor/owner/discovery identities, post-main hosted/Windows receipt and task completion receipt | `SOURCE_CORRECTIONS_CANONICAL / POST_MAIN_INSTALLED_READBACK_AND_COMPLETION_RECEIPT_N.C. / EFFECT0` |
| D1 / TASK-060 | PP-C candidate head `ea5d495` is an ancestor; PP-A/B/C candidate source exists; task status `PP_C_IMPLEMENTATION_CANDIDATE / INDEPENDENT_DEV4_PENDING` | independent DEV-4 acceptance and canonical completion receipt | `CANDIDATE_PRESENT / EFFECT0` |
| D2 / TASK-061 | CA-C candidate head `32c4dde` is an ancestor; CA-A/B/C disabled candidate source exists; task status `CA_C_DISABLED_HISTORY_IMPLEMENTATION_CANDIDATE / REAL_E2E_GATE_PENDING` | bounded real E2E admission correction; secure activation-lock establishment and identity-bound no-overwrite/expected-target config writer; CA-A secure migration lock, identity-CAS journal phases, no-replace Manifest/snapshot commit and owned-temp cleanup; focused Windows race tests; Human activation authority and canonical completion receipt | `DISABLED_CANDIDATE_PRESENT / EFFECT0` |
| D2.5 / SKILL adapter Production transport | canonical SKILL `origin/main=c86ec8c11724a3170d37e0fdc5a516979fcca703` confirms the released adapter/default-disabled historical TASK-058 Evidence while retaining replace-capable publication, unpinned config/delivery/receipt/Profile reads and obsolete fixed-ProgramData/default-config activation instructions | separate SKILL-owner Task/Allowed Files/fresh clean main worktree; code safety plus connector-ready/SKILL workflow/interface/contract/tests/inventory alignment; canonical PR/main/release; installed exact sync/read-back and PL-A baseline hash rebind | `PRODUCTION_LINKAGE_N.C. / EFFECT0` |
...
| --- | --- | --- |
| D0 completion | canonical TASK-063 completion plus corrected installed read-back | missing |
