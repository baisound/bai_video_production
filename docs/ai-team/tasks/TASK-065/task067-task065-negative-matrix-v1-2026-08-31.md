...
| PL65-C01 | adapter `publish-learning`; TASK-036 Product import; BVP pinned verification | PL-B operation ticket and TASK-036 real-installed operation are current | second adapter publish, import repeated, pending result, crash between stage/import/verification | stage exact1 then import exact1; `PENDING` is allowed only before Product verification; terminal `ACCEPTED/DUPLICATE` from BVP proof | exact TASK-036 commit only | exact delivery/correlation/receipt/pending-cleanup delta | exact ordered Profile read-back required | config/history 0 | learning/private/absolute paths 0 | composite receipt binds adapter stage, TASK-036 result, public receipt, hidden correlation and Profile read-back |
| PL65-C02 | PL-C verifier; adapter `canonical_store_written` audit field | Candidate terminal evidence exists | receipt-only, `canonical_store_written`, status-only, missing/wrong Profile or correlation, wrong operation/instance/config | `E2E.N.C. / EFFECT0`; no PASS promotion | 0 | 0 | 0 | 0 | receipt/body/path 0 | negative verifier receipt lists body-free mismatch codes; authority_created=false |
