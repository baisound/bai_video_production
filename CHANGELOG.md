...
- TASK-052として、DbD設計・実装ギャップ監査、分類体系とHuman-first詳細・安全削除、マップ画像decode/回転、Survivor主体Observation/Gold/Teacher契約、transactionalな一括画像登録、発電機・主体別追跡・Survivor状態・フック回数・Killer固有effect・正負Status Effectのprofile-bound時系列状態機械、confirmed Temporal Decisionからadmitted-Evidence CGELへの主体別統合、Killer識別後だけ専用Detectorを選ぶCapability RegistryとGhost Face/Onryo/Doctor starter fixtureを追加しました。別KillerのHUD namespace、識別UNKNOWN、ROI/Evidence欠落、異常stageは専用認識へfall throughせずUNKNOWNになります。実機Windows EXEでネストしたKnowledge詳細が起動を停止する`unhashable type: 'dict'`も修正し、Ownerワークスペースで起動・検索・主要画面操作を確認しています。全体のreal-media packaged acceptance、Provider、Release、Deployは別Gateです。
