...
The main installer sequence is fixed: (1) execute and verify the existing TASK-063 provision/read-back once; (2) execute the TASK-066 helper once; (3) the helper invokes the accepted GF-A InstallLayout resolver/validator, consumes the exact TASK-063 `install_instance_id`, provisions the sidecar and protected writable leaves, writes `<data_root>\settings\installation\task066-installer-readback.json`, and completes its own exact read-back before success; (4) Inno Setup consumes only the helper exit status. The receipt write is containment-checked, ancestor-reparse-safe, regular-file-only, hardlink-rejecting, atomic and no-clobber unless an exact prior Product receipt is validated for repair. Inno Setup never parses the descriptor/receipt or reimplements root/DACL/sidecar rules. Each helper-invoking first-install, `REINSTALL_READBACK`, repair/upgrade or exact journal-recovery operation launches the helper once, waits at most `30 seconds`, never retries automatically and may terminate only the exact helper process it owns; unknown ownership is not killed. The helper permits only those four bounded operation classes and has no network, provider, model, UI, GPU execution, physical user-data deletion or TASK-063 mutation authority. Normal uninstall invokes no TASK-066 helper and performs no TASK-066 mutation.

`settings\installation` is GF-E installer-owned durable state, distinct from the main Settings service profile; the profile writer cannot create, replace or delete lifecycle records. Cache/log cleanup never traverses or deletes it. Normal uninstall deliberately preserves the InstallLayout sidecar, receipt and physical `settings`, `logs` and `runtime-cache` leaves. The binary-root sidecar and all data-root lifecycle/user-data records are outside Inno `[Files]` payload tracking and every uninstall/delete directive. GF-E adds no `[UninstallDelete]` entry and invokes no helper mutation during normal uninstall; physical deletion requires a separate exact cleanup Gate.

First install requires both TASK-066 sidecar and receipt absence after current TASK-063 descriptor validation. `REINSTALL_READBACK` is distinct: after normal uninstall preservation it revalidates, without overwrite, the exact same-instance current TASK-063 descriptor, TASK-066 sidecar and receipt including schema/version, binary-root identity, layout/receipt digests and currentness. Upgrade/repair uses its own prior-receipt transaction. Inno registration never selects the branch.

First-install, upgrade and repair use the durable `<data_root>\settings\installation\task066-installer-transaction.json` journal; cross-root sidecar/receipt creation is not jointly atomic. Its self-hashed envelope binds schema/version, operation ID/phase, `install_instance_id`, canonical binary/data roots, predecessor identities, intended sidecar/receipt hashes, helper/build identity and currentness. The journal is atomically published, flushed and exactly read back before any sidecar or receipt mutation. Journal, sidecar and receipt each use same-directory temp, flush, atomic rename and exact read-back.

Receipt atomic publication plus exact read-back is the sole commit point after sidecar read-back. Only the exact journaled operation may recover: roll forward an absent/matching intended sidecar, remove exact current-operation pre-commit temp/pending artifacts without restoring/deleting/changing any committed predecessor, or clean a post-commit journal after matching receipt+sidecar validation. If exact roll-forward is impossible, recovery fails closed instead of reconstructing or restoring from ambient files. No/foreign/stale/mismatched journal or unbound mixed state has recovery authority. Previous committed receipts are never rollback-deleted. A claimed-first predecessor, one-sided/mixed state without exact journal, missing/stale/cross-instance/substituted/tampered/reparse/nonregular/hardlink or binary-root/layout mismatch fails closed with no ambient reconstruction.
...
Lifecycle tests cover cache-cleanup exclusion, profile-writer mutation0, both-absent first-install, no uninstall delete/helper mutation, exact uninstall→`REINSTALL_READBACK`, exact upgrade/repair, and every journal/sidecar/receipt/commit phase under crash/power-loss/timeout/owned-kill on same- and cross-volume roots. They prove exact journal-only roll-forward or owned-precommit rollback and fail-closed claimed-first collision, unjournaled one-sided/mixed state, missing/stale/cross-instance/substituted/tampered/reparse/nonregular/hardlink/binary-root/layout mismatches.

GF-E also owns the reproducible helper build and payload composition after TASK-063 terminal handoff and exact locks. Root `build-windows-exe.bat` builds `packaging/task066_gpu_runtime_installer.spec` first into unit-scoped clean roots, verifies the fixed-name absolute regular non-reparse/non-hardlink helper plus size/SHA-256, passes it only through `BVP_TASK066_GPU_INSTALLER_HELPER_EXE`, and invokes `packaging/task036_shell.spec`. The main spec independently validates that identity, records the expected hash and COLLECTs exactly one helper. The batch verifies the bundled helper identity/hash and a secret-free bounded smoke; ambient prior output is never an input.

`tools/windows/build-task063-main-installer.ps1` then accepts only the exact freshly built main onedir, verifies exactly one helper plus type/size/hash against the generated identity, computes the payload-tree hash after composition and emits a build receipt binding source HEAD, helper/main spec hashes, helper name/hash/size, canonical PayloadRoot identity, payload-tree hash and installer hash. `task063_main_installer.iss` has an explicit compile-time fixed helper-source check. The joint pre-compile gate rejects missing, wrong-name, directory/reparse/nonregular/hardlink, duplicate, substituted or hash-drifted helper before Inno runs; wildcard inclusion alone is insufficient. Inno never downloads/builds/substitutes or parses TASK-063/TASK-066 JSON semantics. Focused tests cover ordering, clean-root isolation, stale ambient output ignored, all negative cases, receipt binding and GF-F build ownership0. GF-F consumes only the frozen receipt for later native observation.
...
- Post-acceptance build-path delta: source audit found that the current main-installer build script did not build/compose the helper and the ISS wildcard did not fail compilation when it was absent. The source-backed GF-E build/COLLECT/pre-Inno/ISS chain above was accepted by independent delta-2 Critic `0/0/0/0` and delta-2 Judge `ACCEPT`. Implementation remains TASK-063-handoff/currentness/overlap/lock gated.
- Post-acceptance lifecycle delta: the repair predecessor receipt was initially cleanup-eligible beneath `runtime-cache`. The durable installer-owned `settings\installation` binding and explicit `REINSTALL_READBACK` lifecycle above were accepted by independent delta-3 Critic `0/0/0/0` and delta-3 Judge `ACCEPT`. Critic final reader retry was unavailable and used the reviewed patch packet plus prior direct audit; Judge and root retained direct read-back.
- Post-acceptance transaction/recovery delta: cross-volume sidecar/receipt publication is journaled rather than jointly atomic; receipt read-back is the sole commit point; recovery is exact journal-bound roll-forward or current-operation temp/pending cleanup only; normal uninstall invokes no helper and durable records are outside payload/delete ownership. Independent delta-4 Critic `C/H/M/L = 0/0/0/0`; independent delta-4 Judge `ACCEPT`. TASK-063 post-repair native terminal handoff remains a separate GF-E mutation prerequisite.
