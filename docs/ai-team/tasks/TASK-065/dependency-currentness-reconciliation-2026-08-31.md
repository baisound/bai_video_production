...
| D2.5 / SKILL-D2S-001 Production transport | **SUPERSEDED source-reference wording:** the foreign `installer-relative-learning-bridge@c86ec8c` dirty eight-file worktree remains preserve-only and is not a receipt. A dedicated clean-main D2S worktree now has commit `7cdeef2c910d749b561623509c972383fd46faca` on Draft SKILL PR #8: strict bounded/pinned JSON, immutable/no-replace local delivery, closed privacy projection and a data-only operation-config v2 are source-covered. `connector_status` still computes Bridge availability before returning disabled status, and neither canonical main/release/install exact read-back nor a BVP completion receipt exists | land/review the one coherent D2S PR, then require canonical main/release/install exact-byte read-back and a Product-owned completion receipt. Correct disabled `connector_status` to return `DISABLED_LEGACY_SAFE` without Bridge probe/availability projection when `enabled:false`; PL-A rebind only after the released installed result | `D2S_SOURCE_DRAFT_PR8 / D2S-DISABLED-DISCOVERY0 P1.N.C. / RELEASE_INSTALL_READBACK.N.C. / PARTIAL_HUNKS_ARE_NOT_RECEIPT / EFFECT0` |
...
| D2.5 / SKILL learning-export privacy | **SUPERSEDED:** `CLOSED_SKILL_PRIVACY_SOURCE_COVERED` was based on the generic-token draft and is invalid: actor/source/session/ID/style/context/tag values could launder player/account-like text, and caller redaction reports could omit mandatory markers. The bounded D2S PR #8 follow-up changes the projection to closed public codes, exact SHA-256 session digests, adapter-derived opaque IDs, and an exact derived redaction report; its focused re-test/review and Draft-PR source currentness remain pending. BVP must independently validate the cross-process delivery | complete D2S focused privacy/schema/inventory re-test and independent review, then canonical D2S main/release/install exact-byte read-back and independent TASK-058/BVP closed privacy admission before pending/canonical/Profile effect; `safe_export`, public readiness and fixtures remain Evidence only | `PARTIAL_PRIVACY_SOURCE_COVERED / TOKEN_VALUE_LAUNDERING_RETEST.N.C. / REDACTION_REPORT_RETEST.N.C. / BVP_PRIVACY_ADMISSION.N.C. / RELEASE_INSTALL_READBACK.N.C. / EFFECT0` |
| D2.5 / SKILL strict JSON and Product I/O | D2S PR #8 supplies strict bounded UTF-8 parsing, pinned same-snapshot identity binding and local-only no-parent-create/no-overwrite legacy output. The Product side still requires opaque-plan/handle-only I/O and an installed release/read-back; disabled status probing remains separately open | bind projection digest to the adapter's exact pinned parse through the trusted Product broker, close disabled discovery, and prove canonical release/install exact bytes; retain body-free errors and prohibit Product raw config/learning/output paths | `STRICT_JSON_AND_LOCAL_OUTPUT_SOURCE_COVERED / PRODUCT_IO_BROKER.N.C. / D2S-DISABLED-DISCOVERY0 P1.N.C. / EFFECT0` |
