...
def preflight(
    *,
    ...
    resource_binding: dict[str, object] | None = None,
    route: LocalNarrationRouteMode = LocalNarrationRouteMode.ZERO_SHOT_LOCAL,
    ...
    return compile_local_primary_preflight(
        ...
        resource_feasibility_binding=resource_binding or resource(route),
...
def test_unbound_draft_round_trip_schema_mirror_and_public_projection_are_safe() -> None:
    ...
    assert "未導入" in projection["status_message_ja"]
    ...
    private_marker_receipt = compile_quick_clone_readback(
        flow=replace(
            draft(),
    ...
    private_projection = json.dumps(
        public_projection(private_marker_receipt),
        ensure_ascii=False,
        sort_keys=True,
    )
    assert "OWNER_TARO_VOICE" not in private_projection


def test_restart_readback_preserves_central_model_source_and_rejects_selector_forgery() -> None:
    receipt = compile_quick_clone_readback(
        flow=draft(),
        task014_preflight=None,
        calibration_projection=None,
    ...
    restored = QuickCloneReadbackReceipt.from_dict(persisted)
    assert restored == receipt
    assert restored.readback_sha256 == receipt.readback_sha256
    assert public_projection(restored)["canonical_receipt_currentness_state"] == "NOT_BOUND"

    forged = dict(persisted)
    forged["voice_model_selector_present"] = True
    with pytest.raises(ValueError, match="identity"):
        QuickCloneReadbackReceipt.from_dict(forged)

    failed_receipt = compile_quick_clone_readback(
        flow=replace(
            draft(),
    ...
    assert "失敗" in public_projection(failed_receipt)["status_message_ja"]


@pytest.mark.parametrize(
    ("setup_state", "readiness", "message_fragment"),
    [
        (SetupState.INSTALL_APPROVAL_REQUIRED, "BLOCKED", "Owner承認"),
        (SetupState.VERIFYING, "VERIFYING", "確認しています"),
        (SetupState.OFFLINE_BUNDLE_REQUIRED, "BLOCKED", "オフラインbundle"),
        (SetupState.RESTART_REQUIRED, "BLOCKED", "再起動"),
        (SetupState.LOCKED_PRIVATE_DATA, "BLOCKED", "ロック"),
        (SetupState.BLOCKED, "BLOCKED", "ブロック"),
        (SetupState.UNKNOWN, "UNKNOWN", "確認できません"),
    ],
)
def test_setup_recovery_state_has_authoritative_closed_guidance(
    setup_state: SetupState,
    readiness: str,
    message_fragment: str,
) -> None:
    receipt = compile_quick_clone_readback(
        flow=replace(draft(), setup_state=setup_state),
        task014_preflight=None,
        calibration_projection=None,
        synthetic_fixture_request=None,
        synthetic_fixture_receipt=None,
    ...
    assert public["setup_state"] == setup_state.value
    assert public["model_readiness_state"] == readiness
    assert message_fragment in public["status_message_ja"]
...
@pytest.mark.parametrize(
    ("engine_binding", "reason", "readiness"),
    [
        (engine(license_state="LEGAL_REVIEW_REQUIRED"), "MODEL_LICENSE_NOT_COMMERCIAL_ALLOWED", "UNKNOWN"),
        (engine(capability="FAILED"), "MODEL_CAPABILITY_NOT_VERIFIED", "FAILED"),
        (engine(capability="UNKNOWN"), "MODEL_CAPABILITY_NOT_VERIFIED", "UNKNOWN"),
        (engine(state="MISMATCH"), "MODEL_SELECTION_MISMATCH", "FAILED"),
    ],
)
def test_model_license_capability_and_binding_fail_closed(
    ...
    assert public["model_readiness_state"] == readiness
    assert public["execution_enabled"] is False
    assert public["settings_cta_label_ja"] == "中央AI設定を開く"


def test_non_model_preflight_blocker_uses_generic_japanese_guidance() -> None:
    failed_resource = resource(LocalNarrationRouteMode.ZERO_SHOT_LOCAL)
    failed_resource["result"] = "FAIL"
    blocked = preflight(resource_binding=failed_resource)
    receipt = compile_without_fixture(flow_for(blocked), blocked)
    public = public_projection(receipt)
    assert receipt.task014_preflight_decision is PreflightDecision.BLOCKED
    assert public["model_readiness_state"] == "READY"
    assert "実行前確認でブロック" in public["status_message_ja"]
    assert public["settings_cta_label_ja"] is None
    assert public["execution_enabled"] is False

    setup_blocked = compile_without_fixture(
        flow_for(blocked, setup_state=SetupState.BLOCKED),
        blocked,
    )
    blocked_public = public_projection(setup_blocked)
    assert blocked_public["model_readiness_state"] == "BLOCKED"
    assert "音声モデルの準備がブロック" in blocked_public["status_message_ja"]
    assert blocked_public["execution_enabled"] is False
...
def test_serialized_effect_reason_identity_digest_and_unknown_tampering_fail_closed() -> None:
    ...
    oversized["synthetic_fixture_sample_count"] = 48_000 * 61
    oversized["synthetic_fixture_duration_us"] = 61_000_000
    rehash_readback(oversized)
    with pytest.raises(ValueError, match="preview cap"):
        QuickCloneReadbackReceipt.from_dict(oversized)
...
@pytest.mark.parametrize("field", ["flow_reason_codes", "reason_codes"])
@pytest.mark.parametrize(
    "invalid_factory",
    [
        lambda values: {value: None for value in values},
...
def test_readback_wire_reason_codes_require_json_string_arrays(
    field: str,
    invalid_factory,
) -> None:
    exact = preflight()
    record = compile_without_fixture(flow_for(exact), exact).to_dict()
    values = record[field]
    assert isinstance(values, list) and values
    record[field] = invalid_factory(values)
    assert list(validator().iter_errors(record))
    with pytest.raises(ValueError, match="JSON array of strings"):
        QuickCloneReadbackReceipt.from_dict(record)


def test_synthetic_fixture_pair_rejects_bool_integer_forgery() -> None:
    exact = preflight()
    exact_flow = flow_for(exact)
    request = synthetic_request(max_total_frames=True)
    fixture = synthetic_fixture(request, frame_count=1, duration_numerator=1)
    with pytest.raises(ValueError, match="JSON integer"):
        compile_quick_clone_readback(
            ...
            calibration_projection=None,
            synthetic_fixture_request=request,
            synthetic_fixture_receipt=fixture,
            generated_at=NOW,
        )

    request = synthetic_request()
    forged_inputs = deepcopy(request["ordered_inputs"])
    forged_inputs[0]["order_index"] = False
    forged_inputs[0]["pause_after_samples"] = True
    request = synthetic_request(ordered_inputs=forged_inputs)
    fixture = synthetic_fixture(request)
    with pytest.raises(ValueError, match="JSON integer"):
        compile_quick_clone_readback(
            flow=exact_flow,
            task014_preflight=exact,
    ...
    fixture = synthetic_fixture(
        ...
        channels=True,
        frame_count=True,
        duration_numerator=True,
        inserted_silence_samples=True,
    )
    with pytest.raises(ValueError, match="JSON integer"):
        compile_quick_clone_readback(
            flow=exact_flow,
