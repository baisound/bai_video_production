...
- Active Training Studio Follow-up: `TASK-051 / DEV-3 HIGH ASSURANCE`; R0-R6 bounded local gates passed and R7 remains the Windows/package Human Acceptance route. R7N established Game Knowledge review/import and video-analysis export, R7O corrected startup, R7P aligns shared Japanese runtime selectors/Runtime Profile defaults/Analysis+Results/Game Knowledge review UI, and Owner-directed R7Q-R7S now closes the next bounded data-quality route: deterministic known-entity/article classification (including the 11 Owner-verified regression cases), source-kind fallback that cannot overwrite known entities with weak MAP provenance, collector stage timing plus same-run duplicate URL suppression, post-import catalog/Alias timing, and `source_sections` preservation for otherwise-unknown source fields. Local TASK-049/050/051 regression is `367 PASS / 1 Tk-display-only SKIP`; real Windows three-run fetch baseline/median and packaged selector/layout/video-analysis/Game-Knowledge Human Acceptance remain required; no TASK-051 completion, commit, merge or Release is claimed.
