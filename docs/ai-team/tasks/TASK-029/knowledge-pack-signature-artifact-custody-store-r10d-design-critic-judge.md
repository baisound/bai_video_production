...
receipt boundaries. The exact DPAPI cipher instance also has no `__dict__`, so
its encrypt/decrypt methods and suite cannot be shadowed per instance. A direct forced post-init spoof using
...
- focused R10D after second bounded security rework: `19 PASS / 3 Windows-only DPAPI SKIP` on WSL;
- R9B-R10D direct chain: `102 PASS / 4 Windows-only DPAPI SKIP`;
- TASK-029 complete: `180 PASS / 7 Windows-only DPAPI SKIP`;
