    ...
    try:
        ...
        attempts = [
            GenerationAttempt(
                ...
                strategy_level=RegenerationStrategy(int(row["strategy_level"])),
    ...
    def save(path: str | Path, registry: PromptGenerationRegistry, *, expected_previous_snapshot_sha256: str | None = None) -> AtomicWriteResult:
        ...
        if target.exists():
            ...
            if expected_previous_snapshot_sha256 is None:
                raise ProductError("ERR_PROMPT_SNAPSHOT_CAS_REQUIRED", "Replacing a Prompt Registry snapshot requires its exact previous checksum", ProductErrorCategory.AUTHORIZATION)
