...
MODEL_CONFIGURATION_SOURCE = "CENTRAL_AI_SETTINGS"
MODEL_CONFIGURATION_ACCESS = "READ_ONLY"
...
class QuickCloneFlowRevision:
    ...
    def _validate_setup(self) -> None:
        if self.setup_state is SetupState.FAILED and not self.reason_codes:
            raise ValueError("FAILED setup requires reason_codes")
    ...
    def _record_fields() -> tuple[str, ...]:
        return (
            ...
            "model_configuration_source", "model_configuration_access",
...
def readback_currentness_state(revision: QuickCloneFlowRevision) -> str:
    if (
        revision.profile_adoption_state is ProfileAdoptionState.STALE
        or revision.preview_asset_adoption_state is PreviewAssetAdoptionState.STALE
    ):
        return "STALE"
    if (
        revision.execution_state is ExecutionState.UNKNOWN
        or revision.quality_state is QualityState.UNKNOWN
        or revision.runtime_aggregate_state is RuntimeAggregateState.UNKNOWN
        or revision.result_admission_state is ResultAdmissionState.UNKNOWN
    ):
        return "UNKNOWN"
    return "NOT_BOUND"


def ui_guidance_ja(revision: QuickCloneFlowRevision) -> dict[str, Any]:
    if revision.setup_state is SetupState.NOT_INSTALLED:
        status = "音声モデルが未設定です。中央AI設定でモデルを選択してください。"
        next_action = "中央AI設定を開いて音声モデルを設定してください。"
        settings_cta = "中央AI設定を開く"
    elif revision.setup_state is SetupState.VERIFYING:
        status = "音声モデルを確認しています。確認完了まで実行できません。"
        next_action = "確認が完了してから再度状態を読み込んでください。"
        settings_cta = None
    elif revision.setup_state is SetupState.FAILED:
        status = "音声モデルの確認に失敗しました。中央AI設定で状態を確認してください。"
        next_action = "中央AI設定を確認してから手動で再試行してください。"
        settings_cta = "中央AI設定を開く"
    elif revision.setup_state is SetupState.READY:
        status = "音声モデルの準備確認が完了しました。"
        next_action = "実行前提の正本receiptを確認してください。"
        settings_cta = None
    elif revision.setup_state is SetupState.UNKNOWN:
        status = "音声モデルの状態を確認できません。"
        next_action = "正本receiptを再読込し、中央AI設定を確認してください。"
        settings_cta = "中央AI設定を開く"
    else:
        status = "音声モデルの準備が完了していません。"
        next_action = "表示された理由を確認し、中央AI設定で状態を確認してください。"
        settings_cta = "中央AI設定を開く"
    if revision.execution_state is ExecutionState.FAILED_KNOWN:
        status = "音声生成に失敗しました。理由を確認してください。"
        next_action = "理由を解消し、確認後に手動で再試行してください。"
    elif revision.execution_state is ExecutionState.UNKNOWN:
        status = "音声生成の完了状態を確認できません。自動再試行はしません。"
        next_action = "同じ操作の正本receiptを照合して状態を復元してください。"
    elif revision.execution_state is ExecutionState.READY_FOR_QA_REVIEW:
        status = "音声生成が完了しました。品質確認とOwner試聴が必要です。"
        next_action = "生成音声を確認し、品質評価と試聴判断を記録してください。"
    return {
...
def public_projection(value: Mapping[str, Any] | QuickCloneFlowRevision) -> dict[str, Any]:
    ...
    projection = {
        ...
        "reason_code_count": len(revision.reason_codes),
        "has_blocking_reason": bool(revision.reason_codes),
        "canonical_receipt_currentness_state": readback_currentness_state(revision),
        "execution_enabled": False,
        "can_confirm_one_shot": False,
    ...
    projection.update(ui_guidance_ja(revision))
    return projection
