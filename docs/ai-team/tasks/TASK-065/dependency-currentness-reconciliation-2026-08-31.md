...
| D-069 / TASK-069 | negative/currentness matrix is prepared; source has not started and waits for TASK-068 completion. Current `origin/main=35cdf1a` logical tests are historical regression inputs, not Production readiness. TASK-058 directory durability returns unconditional success on Windows and its platform-honesty test accepts a missing directory | after TASK-068, implement and bind FB-R same-open snapshot/ancestor/nlink1, FB-C journal inode CAS/recovery identity, FB-P owned-temp+noreplace+post pinned read-back, FB-X exact-inode cleanup, FB-PR ordered Profile physical identity, FB-DUR native Windows directory durability with unsupported/failure receipt0, PRIV strict bounded closed projection before hash/effect, and READY durable executed `TASK058_BASELINE_READBACK`; canonical review/completion receipt required by TASK-061-A and TASK-065 | `DIRECTORY_DURABILITY_UNOBSERVABLE / START0 / TASK058_PRODUCTION_BASELINE.N.C. / EFFECT0` |
| D0 / TASK-063 | `READ_ONLY_MATRIX_REFINEMENT`: reusable historical tests cover Unicode/custom roots, repair identity preservation, tamper/link/ancestor/concurrency/read-back/rollback cases, fixed-ProgramData literal zero and installer-selected lifecycle contracts. Descriptor absence currently selects the fresh branch even when an owner manifest/old receipt remains, reuses the owner instance ID, and fresh rollback may path-unlink that preexisting receipt before proving operation ownership. TASK-063 directory durability also returns on directory-open failure and suppresses `fsync` failure; no directory-fsync fault fixture exists. The current Inno post-install path does launch/wait for the installed private EXE, but then trusts exit0 plus `installer-readback.json` `FileExists` and ancestor equality without strict receipt body, descriptor/owner/payload or opened physical-identity read-back. `installer_manifest_sha256` is computed from sorted build-input payload paths/hashes and stored by the private EXE; acceptance does not rehash installed files or compare build/descriptor/present payload, uses permissive `ConvertFrom-Json`, and emits absolute install/Bridge roots. GF-D PR #469 has focused PASS; full and hosted-common share one failure parked as a cross-owner old-hash Gate. Montage mutation is zero and TASK-068 is not complete | independently park TASK-068 and GF-D #469; add trusted Product/Installer same-open bounded installed-payload manifest/tree verification, build+installer+descriptor binding, strict receipt-content+identity read-back and path-free public projection, plus I63-R01 descriptor+owner same-open snapshot/mixed-generation rejection, stat-open/read-post different-inode seams, strict nested JSON faults, I63-L01 secure initial/existing lock, temp/target swap seams, Windows native directory durability with unsupported/failure receipt0, descriptor-absent orphan classification with journal-bound exact recovery, operation-owned cleanup identity and foreign-replacement preservation, multi-install/portable/upgrade/uninstall real read-backs and canonical completion receipt | `DIRECTORY_DURABILITY_UNOBSERVABLE / OWNER_ONLY_ORPHAN.N.C. / BUILD_INPUT_CLAIM_ONLY / EXISTENCE_ONLY_PRECEDENT / HISTORICAL_REGRESSION_INPUTS_ONLY / SOURCE_START0 / EFFECT0` |
| D1 / TASK-060 | `START0`: waits for TASK-068 and TASK-063 receipts; older worktrees are clean/merged but are neither reused nor deleted. Current promote/rollback checks exact historical duplicate before CAS and source/policy/confirmation currentness, so stale expected revision may return no-write DUPLICATE | trusted Human challenge; secure PP-B store; fixed Windows DPAPI/Product coordinates; strict outer/decrypted JSON; duplicate limited to historical event ack; fresh pinned current store/source/policy/backend/user/physical identity before PP-C/Profile; private Profile publish/read-back correction and canonical completion receipt | `DUPLICATE_CURRENTNESS_UNPROVEN / PROFILE_CAPABILITY0 / START0 / EFFECT0` |
| D2A / TASK-061-A PREACTIVATION PREPARE | `START0`: waits for TASK-068, TASK-069, TASK-063, TASK-060 and SKILL-D2S-001; this phase closes only the `enabled:false` prepare contract. Current CA-A source has the independent `A61-*` authority/strict-JSON/transaction/snapshot/temp/security/privacy/resource/currentness blockers normalized in the TASK-065 negative matrix | corrected CA-A/B authority, sealed CA-C candidate/challenge, strict same-generation durable snapshots and one PREACTIVATION PREPARE receipt with real-E2E/activation claims zero; no existing plan/snapshot/public readback/terminal journal substitutes | `START0 / PREPARE_AUTHORITY.N.C. / P0_COMMIT_STOP / EFFECT0` |
| D2B / TASK-061-B FINAL CA-C | `START0`: waits for the TASK-036 real-installed E2E receipt. Initial config publication can overwrite a target that appears after the absence check, existing config publication has no opened-bytes/inode CAS, and the transaction publishes before final security/readback and may leave visible mutation with receipt0 (`A61-B-INITIAL-CONFIG-NOREPLACE`, `A61-B-EXISTING-CONFIG-IDENTITY-CAS`, `A61-B-POSTCOMMIT-FAILURE`) | consume the exact TASK-036 receipt, revalidate trusted time/currentness and close final CA-C while retaining `enabled:false`; implement secure initial/existing lock paths, noreplace initial publish, expected opened bytes+identity+revision CAS for existing state, pre/post pinned readback and directory durability. Activation visibility must commit only with Human consume plus final security/readback, or remain an explicit operation-bound pending state. Production Activation remains a separate Human Gate | `START0 / WAITS_TASK036_E2E / P0_ACTIVATION_CONFIG_CAS_AND_COMMIT_STOP / EFFECT0` |
| D2.5 / SKILL-D2S-001 Production transport | `DIRTY_OVERLAP / SOURCE_START0`: `installer-relative-learning-bridge@c86ec8c`, PR0, eight target files dirty. Some default-disabled/null-root concepts are preserve candidates, but the branch is an intermediate whole and owner/disposition is N.C.; two dirty BVP proposal/work-order docs are outside D2S Allowed Files. Canonical source also probes the fixed Bridge filesystem before its disabled decision: write0 is historical PASS but discovery0 is current P1 N.C. | fresh owner disposition; dedicated clean-main D2S worktree; selected concepts reimplemented with strict pinned JSON, operation-v2 ticket/consume, exact-one stage, closed privacy and strict receipt/correlation/Profile; disabled config omission performs probe0 and availability projection0; Product runner requires explicit operation config; full tests, inventory, canonical PR/main/release/install/read-back and PL-A rebind | `D2S-DISABLED-WRITE0 PASS_HISTORICAL / D2S-DISABLED-DISCOVERY0 P1.N.C. / PARTIAL_HUNKS_ARE_NOT_RECEIPT / SOURCE_START0 / EFFECT0` |
...
| D2.5 / TASK-058 File Bridge Production safety | released File Bridge remains canonical historical Evidence; current immutable publication, security-relevant reads, mutable import/Profile journals, ordered Profile pointer/current/marker publication and pending/temp cleanup do not bind opened physical identity against races. Directory durability is a Windows no-op and the current test treats missing-directory success as platform-honest | separate TASK-058-owner corrective Unit with exact paths/symbols and amendment authority, Windows native directory-durability port plus mkdir/pending/receipt/Profile failure fixtures, focused fault/Windows regression, canonical main/release/install exact read-back and downstream baseline rebind | `DIRECTORY_DURABILITY_UNOBSERVABLE / PRODUCTION_LINKAGE_N.C. / EFFECT0` |
...
| D2.5 / TASK-067 | `PRESERVED_DIRTY / COMMIT_STOP`: the exact three-file candidate remains byte-identical and uncommitted; canonical amendment and dependencies are absent. Correlation outcome layering is narrow PASS, but the Bridge-required constructor/get path is create-capable and journal-absent A2 recovery is absent | TASK-068/TASK-069, TASK-060/063 and corrected TASK-061-A PREACTIVATION PREPARE receipts; exact TASK-058 owner-preserving amendment; `T67-READBACK-NOCREATE` and `T67-A2-JOURNAL-ABSENT-TYPED-DUPLICATE` closure; fresh overlap/work-lock PASS; explicit implementation start, DEV-4 review and completion receipt | `T67-OUTCOME-LAYERING-CORRELATION PASS / T67-READBACK-NOCREATE P0.N.C. / T67-A2-JOURNAL-ABSENT-TYPED-DUPLICATE P0.N.C. / COMMIT_STOP / EFFECT0` |
...
| D3 / TASK-065 | `TASK_LOCAL_DESIGN_ACTIVE / SOURCE_START0`: PL-A PLA-I01-I18, PL-B PLB-I01-I17 plus PL65-B03, C/D PLC-I01-I16, PL65-B00/C01a/C01b and PL-D D01-D12 are source-mapped. Public discovery/hash, build-input payload digest, path-bearing acceptance JSON, descriptor timestamps, descriptor-absent owner-only state and current Windows durability no-op are not Product proof; packaged `discover` writes readback; no active-install selector exists | all upstream receipts; trusted zero/one/multiple registration; corrected noncreating reader; same-open descriptor/owner and installed payload manifest/tree; Product EXE/payload; native directory-durability receipts; journal-bound orphan recovery and operation-owned cleanup identity; trusted clock/session and predecessor/successor registration+payload revision propagated through PL-B CAS, TASK-036, PL-C, TASK-061-B and PL-D; accepted implementation Gate and separate Activation Gate. Root scan/timestamp/orphan winner and no-op durability PASS prohibited | `PL_A_START0 / PL_B_START0 / PL_C_START0 / PL_D_START0 / SOURCE_START0 / EFFECT0` |
...
| D0 completion | TASK-063 historical coverage is regression input only; add I63-R01/I63-L01, strict JSON, physical-race/durability, descriptor-absent orphan recovery/owned-cleanup and lifecycle read-backs after TASK-068 and GF-D #469 Gates | read-only matrix refinement / source START0 / missing |
...
| D2 CA-B activation config transaction | `A61-B-INITIAL-CONFIG-NOREPLACE` and `A61-B-EXISTING-CONFIG-IDENTITY-CAS` closed so initial/existing config publication cannot overwrite a raced or physically substituted target | P0 open / Activation PASS0 |
...
| D2.5 SKILL disabled discovery | `D2S-DISABLED-DISCOVERY0`: default-disabled config omission probes no Bridge path and exposes no availability/path; Product runner requires explicit operation config | P1 open / Production linkage PASS0 |
...
| D2.5 TASK-067 | canonical amendment/dependencies; noncreating Bridge-required readback; sealed journal-absent A2 typed duplicate; implementation and focused completion | correlation layering narrow PASS; two P0 N.C.; preserved dirty / COMMIT STOP |
...
| TASK-065 design | Option B plus PLA-I01-I18, PLB-I01-I17, PLC-I01-I16, PL65-B00/B03/C01a/C01b and PL-D D01-D12; one current-installation identity is preserved end-to-end, build-input/acceptance/timestamp/owner-only/no-op-durability substitution is rejected and packaged `discover` is excluded | task-local design active / PL-A/B/C/D START0 / source START0 |
