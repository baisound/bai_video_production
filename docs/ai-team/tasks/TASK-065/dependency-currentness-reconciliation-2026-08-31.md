...
| D2.5 / SKILL learning-export privacy | D2S PR #8 closes the generic public export projection in source: free-form Human reason is local-only, status-derived controlled reason code and exact redacted provenance are emitted, style/context/tags/IDs are bounded tokens, and a post-build validator scans values before `safe_export:true`. Focused privacy/schema/inventory evidence is committed, but the branch is Draft and BVP must independently validate the cross-process delivery | canonical D2S main/release/install exact-byte read-back and independent TASK-058/BVP closed privacy admission before pending/canonical/Profile effect; `safe_export`, public readiness and fixtures remain Evidence only | `CLOSED_SKILL_PRIVACY_SOURCE_COVERED / BVP_PRIVACY_ADMISSION.N.C. / RELEASE_INSTALL_READBACK.N.C. / EFFECT0` |
