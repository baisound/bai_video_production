...
| D2.5 / SKILL-D2S-001 Production transport | **SUPERSEDED as a source-currentness claim:** the old `6a391336` Draft-PR HOLD and foreign dirty-worktree observations remain history only. Canonical SKILL main now contains merged PR #8 at `1646a2e9f3f0cb0a468dd52e564093bde04f49de`, with exact skill-tree SHA-256 `4c3269e00bb934edc15cd58b73eca06c8846b2ed7104e3fa8573e6441ad47dc2` and source-to-main diff `NONE`. These two values are the sole PL-A source rebind input; they are read-only source identity, not an installed-byte, Product-broker, terminal-handoff or E2E receipt | installed-copy exact sync/read-back, TASK-072-owned native broker completion, a versioned Product-owned D2S completion receipt, TASK-036 real-installed E2E and later TASK-061-B trusted recomposition remain independently required. PR head, fixture, public status, source hash alone and copied v2 config are ineligible substitutes | `CANONICAL_SKILL_MAIN_MERGED_ONLY / PL_A_SOURCE_IDENTITY_OBSERVED / INSTALLED_SYNC.N.C. / PRODUCT_BROKER.N.C. / TERMINAL_HANDOFF.N.C. / EFFECT0` |
| D2.5 / SKILL Product-operation authority | D2S PR #8 validates a closed operation-config v2 as data-only/evidence and keeps `authority_created:false`, but independent review confirms H1: `load_operation_config_v2` is not consumed by the public publish path, whose enabled-v1 `--config` route reaches `publish_learning_export` without trusted redemption, `IN_FLIGHT`, burn, expiry or replay closure. An enabled v1 config remains replayable/copyable and indistinguishable from steady state | Product-owned trusted broker/handle route with atomic one-shot redemption, immutable invocation-specific config/receipt coordinates, exact command/instance/input binding, replay/crash closure, release/install exact read-back and PL-A hash rebind; otherwise the TASK-036 preactivation attempt is internal synthetic-only and cannot produce real-installed E2E | `V2_DATA_ONLY_EVIDENCE_ONLY / H1_PREACTIVATION_AUTHORITY.N.C. / EFFECT0` |
...
| D2.5 / SKILL learning-export privacy | **SUPERSEDED:** generic bounded tokens were insufficient: actor/source/session/ID/style/context/tag values could launder player/account-like text, and caller redaction reports could omit mandatory markers. D2S `a99935d` adds closed public codes, exact SHA-256 session digests, adapter-derived opaque IDs and an exact derived redaction report, but independent review found M3: unknown music/video public context fields are silently dropped rather than failing closed as the interface contract requires. BVP must independently validate the cross-process delivery regardless | correct M3 and re-run privacy negatives, then require canonical D2S main/release/install exact-byte read-back and independent TASK-058/BVP closed privacy admission before pending/canonical/Profile effect; `safe_export`, public readiness and fixtures remain Evidence only | `SKILL_PRIVACY_PARTIAL / M3_UNKNOWN_PUBLIC_FIELD.N.C. / BVP_PRIVACY_ADMISSION.N.C. / RELEASE_INSTALL_READBACK.N.C. / EFFECT0` |
| D2.5 / SKILL strict JSON and Product I/O | D2S PR #8 adds strict bounded UTF-8 parsing, same-snapshot identity checks, local-only no-parent-create/no-overwrite legacy output and disabled status with bridge-root/availability projection zero. Independent review found H2: ancestor checks are path/lstat rechecks rather than full handle-pinned nofollow ancestry, leaving Windows reparse/ABA exposure; M1 also places the enabled/bridge-root conditional in the wrong schema location. The Product side still requires opaque-plan/handle-only I/O and an installed release/read-back | correct H2/M1, add the required Windows ancestor/fault negatives and obtain Windows 53/53 evidence; then bind projection digest to the adapter's exact pinned parse through the trusted Product broker and prove canonical release/install exact bytes | `STRICT_JSON_PARTIAL / H2_HANDLE_PINNING.N.C. / M1_SCHEMA_DRIFT.N.C. / PRODUCT_IO_BROKER.N.C. / EFFECT0` |
