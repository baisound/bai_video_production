...
document.querySelectorAll('.workspace').forEach(b=>b.addEventListener('click',async()=>{await call('set_workspace',{workspace:b.dataset.w});await refresh();const queueDrawer=document.querySelector('#generationQueueWorkspace'),audioDrawer=document.querySelector('#audioWorkspace');if(b.dataset.w==='GENERATION_QUEUE'){renderGenerationQueue(await call('generation_queue_snapshot'))}else{queueDrawer.hidden=true}if(b.dataset.w==='AUDIO_WORKSPACE'){renderAudioWorkspace(await call('audio_workspace_snapshot'))}else{audioDrawer.hidden=true}}));
