...
- Latest Phase G regression: WSL2 Ubuntu `791 passed`; the earlier downstream Windows checkpoint passed `782` with one intentional non-Windows skip. The updated PyInstaller one-dir bundle includes the pre-edit/Product-port/post-review runtime graph and passes native top-level-window launch and normal close.
