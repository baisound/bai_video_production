...
- Project Status: `V0_22_0_RELEASED / TASK_036_P_UX_1C_VISUAL_HOSTED_CLOSED / TASK_036_P_UX_2_D4_D5_HOSTED_CLOSED / TASK_036_P_UX_2E_NATIVE_CLOSURE_PENDING / TASK_046_VOICE_ROUTE_SEPARATE / TASK_047_P_OBS_1_PRODUCTION_RECORDING_P0_GATE / TASK_049_GAME_INTELLIGENCE_INTEGRATED / TASK_050_TRAINING_STUDIO_FOUNDATION_INTEGRATED`
- Last Completed Release Unit: `TASK-051 — v0.22.0`; release PR #191 merged at exact release-code main `50a2f06b4d5b64764a521c5863aa1632992a1418`, annotated Tag and stable GitHub Release verified
...
- P0 Production Recording Dependency: `TASK-047 / P-OBS-1 TECHNICAL FOUNDATION HOSTED CLOSED`; the P-OBS-1A contract, installer, Release composition/public Technical Preview route and selected-source Plugin/runtime/Controller passed PR #102/#119/#125/#127/#132. This closure does not admit production training material: no production recording, private audio persistence, Dataset adoption or fine-tuning may start before a fresh exact-path/runtime probe, recording Consent, verified encrypted storage and Owner GO; P-OBS-2 continuous/multi-source breadth remains later.
...
- Owner-routed Game Intelligence Lane: `TASK-049 DbD Game Intelligence / Canonical Game Event Timeline`; R1-R10A, R10B0 and R10B1-R10B5D bounded recognition/knowledge/LLM/trivia/training-intake/HUD-calibration/data-migration/Community-Reference collection baselines are locally implemented. The V6 Game Intelligence Workspace remains additive. Windows packaged execution and real-media Human Gold accuracy evidence remain open. A standalone Game Intelligence product is not planned; bounded `BAI DbD Trivia Editor.exe` and `BAI DbD Training Studio.exe` utility build definitions are implemented.
- Completed Training Studio Foundation: `TASK-050`; PR #186 merged. The completed Task remains historical and is not reopened.
- Completed Training Studio Follow-up: `TASK-051 / v0.22.0`; release closure is complete and PR #196 merged the broad DbD Training Studio integration. The completed Task remains historical and is not reopened.
- Active DbD successor: `TASK-052 / DEV-3 HIGH ASSURANCE`; explicit Owner AUTONOMY implementation authority covers the bounded implementation lane. R0/R0A established the 74-row baseline; R1A-R1C completed taxonomy/parser, Human-first details/safe delete and map preview hardening; R2A/R2B completed Survivor-scoped identity and transactional background registration; R2B-F1 fixed the packaged `unhashable type: 'dict'` startup failure; R3 completed profile-bound temporal state and admitted-Evidence integration; R4 completed Killer Capability/recorded-video/Teacher routing; R5 completed positive/negative Status Effect segmentation, identity, Teacher, Gold and temporal integration. Detailed per-unit evidence remains in `docs/ai-team/tasks/TASK-052/task.md`.
- TASK-052 latest continuation: R6 object/scene recognition, R7 canonical selective Vision planning and R8 Human Gold/KPI/correction contracts are complete. Final Critic requires real media/rights/Human labeler provenance, rejects secret-like refs and prevents claim authority from overriding failed KPI thresholds. R9 exact Windows EXE build/startup, bounded Japanese UI, cached PNG pixel rotation/reopen/restart and isolated native backup/restore contracts passed; the temporary orientation was restored. Implementation is complete through R9 for available Evidence. Multi-Killer/UI-scale real video, generator-0 performance, complete Human Gold and production accuracy remain `NOT_CONFIRMED` behind the Human/data gate.
- Active DbD implementation: `TASK-054 / DEV-3`; Owner exact instruction `実作業に戻って` on `2026-08-21` authorizes the bounded local implementation lane under the standing Atomic Unit rule. R0 contracts/threat model, R1 deterministic Context Assembly and R2 Output Admission/Human lineage are merged on `main` through PR `#264`; R3A Binding Registry is merged through PR `#267` and its CHANGELOG lock is closed through PR `#271`. R3B pure Route Capability is commit-ready on PR `#273` with `8 / 9 PASS` and only the shared CHANGELOG gate pending behind the active TASK-036 reservation. R3C deterministic fake adapter/fault harness is locally complete and commit-ready: every emission revalidates current R3B state, uses only the canonical R2A parser, retains no raw fixture on the returned Attempt and remains `TEST_ONLY_NO_PROVIDER_EXECUTION`. R4A independently adds a body-free Dataset rights/provenance manifest fixed to `CANDIDATE_ONLY_NO_ADOPTION`, with opaque hash references and source-group split isolation. R3C focused Evidence is `18 PASS`; R3C + R3B + R2A direct boundary Evidence is `160 PASS`; TASK-054 plus TASK-049 direct regression is `398 PASS`; R4A focused Evidence is `9 PASS` and its direct R0/R2D lineage set is `85 PASS`. Dataset adoption, narration intake, binding approval authority, model/runtime download, training, Provider execution/R3D, TTS, Timeline adoption, Product Activation, release and deploy remain Human-Gated.
- TASK-054 local R4B continuation: the narration intake candidate contract is commit-ready. It reuses R4A rights eligibility and R2C DLP, binds exact media range, ASR/diarization revisions, pseudonymous speaker, CGEL Event/Context and Human review coordinates, retains only Human-reviewed redacted text plus original digest, and remains `CANDIDATE_ONLY_NO_ADOPTION`. R4B/R4A/R2C focused Evidence is `80 PASS`; real media intake, Dataset adoption, training and Provider execution remain Human-Gated.
- TASK-054 local R5A continuation: the Training Studio now keeps an explicit Japanese `確認モード（学習しない）` / `学習モード` control visible across tabs and records each explicit selection in an append-only checksum-bound Workspace receipt. Confirmation remains the no-write default; active operations block switching; tampered/foreign receipts fail closed. Learning selection is preparation eligibility only and explicitly grants no training, Provider, Dataset or Binding mutation authority. R5A plus R0 focused Evidence is `47 PASS`; TASK-054 plus direct TASK-049 and OSS boundary regression is `647 PASS`; compileall, Schema mirror and diff checks pass. R5B time-aligned ordinary-video commentary preview is next; actual inference, Dataset adoption, training, model/runtime acquisition, promotion, release and deploy remain Human-Gated.
- TASK-054 local R5B continuation: a read-only time-aligned Commentary Preview now compiles exact TASK-049 Game Intelligence exports and validated Candidates without creating another Timeline/store. It verifies outer and nested checksums, exact Match/Event fields, crossing/duplicate/orphan bounds and video duration; only CONFIRMED admitted-review Events render. The Japanese panel exposes timing, category, text, confidence, validation, prior/next, `前後10秒`, and `解説あり / 解説なし`, with permanent no-learning/no-model-change copy. Empty output and unbound local video identity remain explicit NOT_CONFIRMED states. R5B/direct TASK-049/R5A focused Evidence is `26 PASS`; TASK-054 plus direct TASK-049 and OSS boundary regression is `656 PASS`; compileall, Schema mirror and diff checks pass. R5C status/execute/review connection is next; R5B performs no inference, TTS, Dataset/model/training/Provider/Timeline/Resolve mutation.
- TASK-054 local R5C continuation: the Japanese model status/preflight/execute/review panel now projects current R3A Binding lifecycle and R3B Route capability without another model source. It displays Japanese/schema, rights/evaluation and truthful GPU-unknown states; preflight errors remain actionable. Even a successful preflight remains `NOT_AUTHORIZED_R3D_REQUIRED`, so `現在の実況・解説を確認` is structurally disabled until a separate R3D authority contract exists; review requires a positive pending count. R5C/R3B/R5B focused Evidence is `43 PASS`; TASK-054 plus direct TASK-049 and OSS boundary regression is `663 PASS`; compileall and diff checks pass. No Provider/model execution, TTS, Dataset/training or Timeline/Resolve effect occurs.
- TASK-054 local R4C continuation: the Dataset leakage audit is commit-ready. Exact R4A/R4B re-admission, source-group/Match split isolation, corrected-transcript duplicate and normalized phrase-overlap detection are bounded and body-free. The canonical report and mirror reject tampering and remain `EVIDENCE_ONLY_NO_ADOPTION`. R4C/R4B/R4A focused Evidence is `27 PASS`; TASK-054 plus direct TASK-049 and OSS boundary regression is `610 PASS`. Dataset adoption, training and Provider execution remain Human-Gated.
- TASK-054 R4D continuation: the offline BASELINE/GENERIC/TUNED evaluator is on PR `#311`; Security and six-platform CI are `8 / 9 PASS`, while the required CHANGELOG check is parked behind the canonical nonclosed TASK-036 P-UX-2L shared reservation. It requires exact PASS R4C, one held-out cohort, canonical arm order and fixed seeds `104729 / 130363 / 155921`; applies non-compensating schema/fact/patch/citation/privacy/split/replay/safe-negative gates; and remains `EVIDENCE_ONLY_NO_PROMOTION`. Direct Evidence is `18 PASS`; targeted regression is `619 PASS`. No unauthorized CHANGELOG write occurred.
- TASK-054 local R4E-A continuation: body-free blind Human-review presentation, separately sealed per-sample A/B/C-to-arm reveal mapping, and exact one-shot Human submission Evidence are commit-ready. Presentation cannot expose arm/model/binding identity; reveal and submission admission require an exact externally bound, time-limited Human authority record and reject R4D/sample/candidate crossing, secret-like refs, non-blind reason codes and checksum forgery. R4E-A/R4D/R2 focused Evidence is `45 PASS`; TASK-054 plus direct TASK-049 and OSS boundary regression is `629 PASS`. Actual Human review collection, R4E-B aggregation/promotion candidate, budget acceptance and promotion remain separate Human Gates.
- TASK-054 local R4E-B continuation: blind-review aggregation and a body-free promotion-candidate report are commit-ready. Every submission is admitted while blind, then exact reveal mapping is applied; full sample/reviewer coverage, unique one-shot confirmations and exact submission/Authority-set digest are required. Factual regression and unsupported style improvement are non-compensating; agreement below 500/1000 is `NOT_CONFIRMED`. Even `READY_FOR_OWNER_REVIEW` remains `PROMOTION_CANDIDATE_ONLY_OWNER_DECISION_REQUIRED`. R4E-B focused Evidence is `8 PASS`; R4E-B/R4E-A/R4D is `27 PASS`; TASK-054 plus direct TASK-049 and OSS boundary is `637 PASS`. Actual Human review, real-evidence candidate issuance, budget acceptance and promotion remain Human-Gated.
- Owner priority creator-workflow insertion: `TASK-056 / DEV-3 — Chase Keyword Cue / Semantic Audio Cue Bridge`; R0 core and R2 external SKILL integration are merged. R1 Product integration is locally complete on `codex/task-056-r1-product-integration`: existing P-UX-2K transcription supplies word timing, a fixed Project Application Service creates text-free cue publications, and the Edit GUI provides per-cue `ACCEPT / REJECT` through prepare/confirm/apply/cancel. Human decisions are immutable, atomically stored, restart-readable and Schema-bound without confirmation tokens, transcript text or host paths. Base Evidence/CONFIRMED_ONLY sidecar remain unchanged; no Timeline/Resolve/auto-apply authority. R1A full regression is `3625 PASS / 5 SKIP`; R1B focused verification is `135 PASS`; R1 final full regression is `3628 PASS / 5 SKIP`; hosting is next.
- TASK-049 local verification: `CGEL / Store / adapters / event resolver / Perk Knowledge / Review / V6 Workspace / Commentary Validator / proposal bridge / analysis export / benchmark / native pilot / HUD+Item-Addon+Perk+OCR+Killer-Power recognition baselines / Cross-modal Fusion / explicit-authority LLM adapter / Trivia Knowledge / GUI Training Studio intake / versioned HUD Calibration / anchor alignment / Item-Add-on loadout recognition / DbD data migration / Kamigame Knowledge Candidate Import` are implemented locally. Production accuracy is NOT claimed until real-media Human Gold evaluation passes; Windows EXE builds are NOT_CONFIRMED on the current Linux host.
- TASK-049 latest broad regression: `2140 PASS / 1 Windows-only SKIP / 0 FAIL / 0 DESELECT` across all `tests/test_*.py`; the skip is the pre-existing TASK-047 Inno Setup acceptance gate that requires Windows.
...
- TASK-035 REAPER Audio Finishing Foundation R0 is hosted-closed: the body-free DAW capability, deterministic Session Plan, project snapshot, one-shot Human authorization, external execution/QA binding and append-only Audio Round-trip manifest contracts passed the local metadata-only gate; PR #147 exact head `8017d8e866fd744471c6eb204782dc0ed43d88a6` passed all `9 / 9` hosted checks and merged at exact main `eb1ea970cecda1cffb3577dde6efc99b4958582d`. REAPER/iZotope launch or mutation, audio read/render/analysis, Asset promotion, Resolve placement, Release, Deploy and Production remain blocked by their separate runtime, license and Human gates.
- TASK-048 Voice Quality Calibration canonical foundation is hosted-closed: P-QC-1A defines the pure calibration metadata, append-only revision, privacy and Human-decision boundaries (PR #106; exact main `458b671fb2a00e0ec820edde4e6cefea6b766059`), while P-QC-1B admits TASK-047 local GAIN receipts without promoting them to quality PASS (PR #136; exact main `82c9191791a76a1cc76784e01a12816899cebc9a`); both passed `9 / 9` hosted checks and their locks closed through PR #108/#137. Real audio/analyzer/hardware execution, canonical quality receipt issuance, Dataset/Asset promotion, Training/Model use and Auto Approve/Reject remain separate fresh Gates.
...
- TASK-014 Local Primary supply-chain foundations are hosted-closed through the render-admission contract (PR #153), Qwen payload observation and locked-wheel session (PR #193), pinned model manifest and point-in-time snapshot diagnostics (PR #195/#197/#198), and the proposed `packaging` parser verifier plus one bounded metadata observation (PR #204/#207). Every persisted receipt remains diagnostic and non-capability; official pin acceptance, any further metadata GET, artifact-body acquisition, parser/resolver use, target Python or model load, audio/inference, Owner voice processing and paid/Cloud execution remain separate fresh Gates.
...
- TASK-013 provider-neutral BGM and SFX routing compilers are hosted-closed: canonical Profile/Asset/rights/capability/license/resource Evidence is reduced to deterministic body-free route plans while UNKNOWN remains fail-closed. BGM PR #140 exact head `2c19f85e9de500199084fd66364b4c9eddd2278d` passed all `9 / 9` hosted checks and merged at exact main `069df955e5698baf50b5e18f940bf0665035e625`; SFX PR #141 exact head `1c84c9ec81911b43fbd7fe64d644bb4f43bd6965` passed all `9 / 9` hosted checks and merged at exact main `404149a1c8d9bb722f9bdaa2ce7ba9d8ee7a23cf`. Provider calls, Credential resolution, paid/local generation, audio/media I/O, Asset publication, placement, Resolve, Release and Deploy remain blocked by their separate gates.
- TASK-026 P-AUDIO-1 Product promotion is hosted-closed: append-only Product placement-plan history, restart-derived CURRENT/STALE projection, exact upstream re-derivation, TASK-043 coordinated save and the narrow Audio Workspace persistence action passed focused `94 / 94`, full WSL2 `1157 / 1157`, schema/mirror and compile gates; PR #86 exact head `a907d199a0f70cf05dc24361f512d84cd71163f6` passed all `9 / 9` hosted checks and merged at exact main `0e457e697a8099eac885d7edb88d5e77b0eca431`. Provider/paid execution, audio generation or media writes, TASK-010 execution and Resolve/Cubase mutation remain blocked by separate gates.
...
- TASK-041 Audio Workspace Media Review / Handoff Foundation R0 is also hosted-closed: body-free source/review coordinates, external playback/waveform receipts, independent audio/visual Human decisions, non-destructive derived-Asset proposals and TASK-035 round-trip states passed the local metadata-only gate; PR #159 exact head `a7bed99f7d3cb38500640160540c731641a204e1` passed all `9 / 9` hosted checks and merged at exact main `3d737d950be72c34ce318ec768e57a5b21c346d5`. Audio reads/playback, waveform or media generation, Asset publication, placement mutation, DAW effects, Release and Deploy remain blocked by their separate gates.
