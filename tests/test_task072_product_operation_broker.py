...
TASK072_R1_DESIGN = ROOT / "docs/ai-team/tasks/TASK-072/op-ticket-core-v1-detailed-design.md"
ACCEPTED_PARENT_DESIGN_SHA256 = (
...
def test_fixture_action_matrix_is_closed_and_non_authoritative() -> None:
    ...
    design_text = TASK072_R1_DESIGN.read_text(encoding="utf-8")
    assert "TASK072-A-OP-TICKET-CORE-V1-R1" in design_text
    assert "52203bc9962340016f4b7ac494ea02d25202484d" in design_text
    assert ACCEPTED_PARENT_DESIGN_SHA256 in design_text
    assert value["design_sha256"] == ACCEPTED_PARENT_DESIGN_SHA256
...
def test_ticket_schema_accepts_exact_coordinates_and_rejects_invalid_vectors() -> None:
    ...
    for coordinate in vectors["invalid_event_coordinates"]:
        ...
        assert list(validator.iter_errors(candidate))
    for mutation in vectors["invalid_state_correlations"]:
        candidate = {**vectors["valid_first_event"], **mutation}
        assert list(validator.iter_errors(candidate))
